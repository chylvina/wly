...
def load_scan(path):
    ...
    #         print('slices:', len(slices), '- sec_num:', sec_num)
    ...
    # print(slices[0].ImagePositionPatient[2],slices[1].ImagePositionPatient[2],slices[0].SliceLocation,slices[1].SliceLocation)

    for s in slices:
        s.SliceThickness = slice_thickness

    return slices, ins_dict,isflip
...
def load_dicom2(path):
    slices, ins_dict, isflip = load_scan(path)
    instances = [s.InstanceNumber.__str__() for s in slices]
