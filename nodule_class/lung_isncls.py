...
def pbb_to_df(labels, spacing, extendbox, mask):
    ...
    for i, label in enumerate(labels):
        # print(label[:3][::-1],mask.shape)
        if not (label[:3][::-1] < np.array(mask.shape)).all():
            continue
        data.append([label[0], label[1], label[2], label[3], prob[i]])
    # labels = np.column_stack((labels, prob))
    return pd.DataFrame(data, columns=['coordX', 'coordY', 'coordZ', 'diameter_mm', 'probability'])
