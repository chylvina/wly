    ...
    def run(self):
        ...
        while True:
            try:
                ...
                'GPUThread get queue_det: ', e)
            try:
                t_s = time.time()
                nodule_df = self.lung_dete.prediction(result_dict['prep_data'], result_dict['prep_spac'],
                                                      result_dict['prep_ebox'],result_dict['prep_mask'])
                print(time.ctime(), ' ', i, 'GPU DOING USE TIME(lung dete prediction):', time.time() - t_s)
                t_s = time.time()
                preb = self.lung_isnc.nodule_cls(nodule_df, result_dict['prep_case'], result_dict['prep_spac'])
                print(time.ctime(), ' ', i, 'GPU DOING USE TIME(lung isnc nodule cls):', time.time() - t_s)
                ...
                try:
                    ...
                    'GPUThread put queue_ret: ', e)
                print(time.ctime(), ' ', i, 'GPU DOING US TIME(lung lobe):', time.time() - t_s)
                i += 1
